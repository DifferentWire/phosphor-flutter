...
PROJECT_DIR="/c/Dev/phosphor-flutter"
# ──────────────────────────────────────────────────────────────────────────
...
# ─── 3. Git Hooks Path ──────────────────────────────────────────────────
# Mode-aware (#221) + worktree-safe + fail-loud (#239). In untracked-governance
# repos (.claude/ gitignored, e.g. public forks) worktrees do NOT carry .claude/,
# so a RELATIVE hookspath resolves to a nonexistent dir in every worktree and all
# lifecycle hooks silently stop firing (OffbandMesh/meshcore-client#384). The path
# must be ABSOLUTE into the MAIN clone. Detection MUST key off the MAIN clone, not
# the current dir: a worktree has no .claude/settings.json, so a per-worktree
# check-ignore mis-reads the mode as 'tracked' and re-sets the relative path (shared
# config) — re-breaking every worktree. core.hookspath is repo-level, so one correct
# absolute value fixes all worktrees at once.
MAIN_CLONE=$(cd "$PROJECT_DIR" && dirname "$(git rev-parse --path-format=absolute --git-common-dir 2>/dev/null)" 2>/dev/null)
[ -d "$MAIN_CLONE" ] || MAIN_CLONE="$PROJECT_DIR"
if (cd "$MAIN_CLONE" && git check-ignore -q .claude/settings.json 2>/dev/null); then
  WANT_HOOKS_PATH="$MAIN_CLONE/.claude/hooks"
  HOOKS_MODE="untracked-governance: absolute -> main clone (all worktrees inherit)"
else
  ...
  if ! (cd "$MAIN_CLONE" && git ls-files --error-unmatch .claude/settings.json >/dev/null 2>&1); then
...
if [ "$(norm_path "$HOOKS_PATH")" != "$(norm_path "$WANT_HOOKS_PATH")" ]; then
  warn "Git hooks path was '${HOOKS_PATH:-<unset>}' — set to $WANT_HOOKS_PATH ($HOOKS_MODE)"
  cd "$PROJECT_DIR" && git config core.hookspath "$WANT_HOOKS_PATH"
fi
# Fail-loud integrity guard (#239): whatever core.hookspath ends up as, the
# resolved directory MUST contain the hooks — otherwise every commit/push gate
# silently no-ops. This catches worktrees a stale relative path re-broke, and
# any repo whose hookspath was never fixed. Resolve relative paths against this
# working tree's root. Only fail when the MAIN clone actually HAS hooks to run
# (a truly fresh, not-yet-bootstrapped repo has none — that's provisional, above).
RES_HP=$(cd "$PROJECT_DIR" && git config core.hookspath 2>/dev/null || echo "")
case "$RES_HP" in
  /*|[A-Za-z]:*) HP_DIR="$RES_HP" ;;
  "")            HP_DIR="" ;;
  *)             HP_DIR="$(cd "$PROJECT_DIR" && git rev-parse --show-toplevel 2>/dev/null)/$RES_HP" ;;
esac
if [ -f "$MAIN_CLONE/.claude/hooks/pre-commit" ] && [ ! -f "$HP_DIR/pre-commit" ]; then
  fail "GIT GATES ARE SILENTLY OFF here — core.hookspath ('${RES_HP:-<unset>}') resolves to a dir with no pre-commit. Every commit/push bypasses Citadel/issue-link/branch/reservation/close checks."
  log "  Fix: git -C \"$PROJECT_DIR\" config core.hookspath \"$MAIN_CLONE/.claude/hooks\""
else
  pass "Git hooks path ($HOOKS_MODE) — hooks resolve"
fi
